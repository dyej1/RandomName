...
int main(void)
{
    ...
    // Define the distance in between data points. The period is 0.02 sec and there are 50 periods
    fRadians = ((0.02) / (SERIES_LENGTH / 50));

    // Calculate and store all of the desired data point values
    while(i32DataCount < SERIES_LENGTH)
    {
        // Calculated the value for each point on the sine wave
        gSeriesData[i32DataCount] = 1.0 * sinf(2 * M_PI * 50 * fRadians * i32DataCount) + 0.5 * cosf(2 * M_PI * 200 * fRadians * i32DataCount);
